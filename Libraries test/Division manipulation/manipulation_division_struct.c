...
char *string_creation(int * error_func){
	// Function's code
	char *string = NULL;
	string = (char *) malloc(sizeof(MAX_CHARACT_STRING));
	if(NULL == string){
...
void insert_info_division_struct(division **system_divisions, int number_divisions, int *error_func) {
	...
	int counter1, counter2, aux_virgula = 0, counter3, counter4 = 0, counter5 = 0, counter6;
	char str_file[SIZE_STRING_BUFFER_1] = {0};
	char str_aux1[SIZE_STRING_BUFFER_1] = {0}, str_aux2[SIZE_STRING_BUFFER_1] = {0};
	...
	while(fgets(str_file, SIZE_STRING_BUFFER_1, sensor_configuration) != NULL) {
		for(counter6 = 0; counter6 < number_divisions; counter6++) {
			...
			for(counter2 = counter1; ';' == str_file[counter2]; counter2++) {
				str_aux2[counter2] = str_file[counter2];
			...
			for(counter3 = 0; '\0' != str_aux2[counter3]; counter3++) {
				system_divisions[counter6]->sensors[counter4][counter5] = str_aux2[counter3];
				if(',' == str_aux2[counter3]) {
					system_divisions[counter6]->sensors[counter4][counter5] = '\0';
					counter4++;
					counter5 = 0;
				}		
			}
		}
		printf("%s\n", system_divisions[counter6]->division_name);
	
		for(counter1 = 0; counter1 < aux_virgula+1; counter1++)
		{
			printf("%s\n", system_divisions[counter6]->sensors[counter1]);
		}	
	}
}	
